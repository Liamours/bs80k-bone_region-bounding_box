...
- Whether a per-crop relative threshold, for example a percentile of that crop's own pixel values rather than one fixed number for every crop, avoids the empty-mask failures seen above and does any better on shoulder, not tried yet
- Shoulder's actual problem is still unexplained. Ruled out so far: search ambiguity alone, the head-to-pelvis band, background masking at threshold 2, and now higher fixed thresholds too
...
Baseline template matching is implemented and has been run on a 20-id sample across all 26 region folders, see "Baseline results" above. It works essentially perfectly for 18 of 26 folders. Vertebra's lower score is explained, a real shape mismatch against a fully textured, non-rectangular crop. Masked template matching, `src/matching/masked_template_match.py`, fixes chest well, near-exact fraction and ssim roughly double, see "Masked template matching" above. Shoulder is still open and has resisted every fix tried so far, a head-to-pelvis search band, background masking at the original threshold, and a sweep of higher thresholds, see "Mask threshold sweep" above, which also surfaced a separate empty-mask failure mode in high thresholds worth remembering for later. Not yet run on the full dataset. `src/eval/crop_match_metrics.py`, the match scoring module used throughout, is implemented and its own self test has been run against synthetic arrays, see the section above that.
