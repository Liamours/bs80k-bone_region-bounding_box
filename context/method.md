...
- Shoulder's actual problem is still unexplained. Ruled out so far: search ambiguity alone, the head-to-pelvis band, background masking at threshold 2, higher fixed thresholds, and now relative percentile masks at several levels, all either flat or actively worse. A geometric mismatch template matching cannot express, translation only search against a crop whose true content may involve some rotation or warp, is untested and is the one major remaining category of explanation
...
Baseline template matching is implemented and has been run on a 20-id sample across all 26 region folders, see "Baseline results" above. It works essentially perfectly for 18 of 26 folders. Vertebra's lower score is explained, a real shape mismatch against a fully textured, non-rectangular crop. Masked template matching, `src/matching/masked_template_match.py`, fixes chest well, near-exact fraction and ssim roughly double, see "Masked template matching" above. Shoulder is still open and has resisted every masking based fix tried so far, a head-to-pelvis search band, fixed thresholds low and high, and now relative percentile masks too, see "Mask threshold sweep" and "Percentile mask sweep" above. Excluding more background consistently makes shoulder worse rather than better, unlike every other region tried, so the remaining open explanation is a geometric mismatch, not an amount-of-background one. Not yet run on the full dataset. `src/eval/crop_match_metrics.py`, the match scoring module used throughout, is implemented and its own self test has been run against synthetic arrays, see the section above that.
