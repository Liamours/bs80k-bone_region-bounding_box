...
`generate_bounding_boxes.py` has since been updated to use the vertebra anchored band for shoulder specifically, once that approach was validated at full scale below, rather than the plain full image search every other folder uses. The deliverable was regenerated after that change, so `bounding_boxes.csv` now reflects the better shoulder approach, not the original one this section first described.
