...
24 of 26 folders are solved and confirmed reliable. Shoulder is the one unresolved folder, though anchoring its search on vertebra's own matched position, confirmed at full dataset scale, cuts its positional outlier rate roughly in half to two thirds, anterior view is now close to every other folder, posterior view is much improved but still elevated. What limits it further is a harder, more intrinsic precision problem within the right neighborhood, tied to how little real signal the shoulder crop actually contains, and to shoulder being a junction between other structures rather than a distinctive bony feature of its own, confirmed by looking at the base algorithm paper's own figures directly. Detail and backing for every row is in the sections that follow.
...
| 19 | Looked at ref 37's own figures directly | How does the base algorithm paper actually draw the shoulder region | Its shoulder has the same diagonal neck cut seen in this project's own crops, not an artifact, and it sits in a junction zone bordered by head, humerus, vertebra, and thorax, not a self contained bony feature like pelvis or vertebra |
| 20 | Vertebra anchored search, full dataset | Does the outlier rate itself drop at full scale, not just the 20 id sample | Yes, outlier rate roughly halved to two thirds cut, anterior view now close to every other folder, posterior view improved but still elevated |
...
`generate_bounding_boxes.py` has since been updated to use the vertebra anchored band for shoulder specifically, once that approach was validated at full scale below, rather than the plain full image search every other folder uses. The deliverable was regenerated after that change (first attempt hit a transient out of memory error partway through, unrelated to the approach itself, a retry completed cleanly), so `bounding_boxes.csv` now reflects the better shoulder approach, matches the standalone validation numbers exactly, not the original one this section first described.

`src/matching/add_labels.py` then joined each row's own normal/abnormal label from that component's own txt label file (`context/dataset.md`) onto the same `bounding_boxes.csv`, adding `label` (0 or 1) and `diagnosis` (normal or abnormal) columns, 16 columns and 76050 rows total, every row got a label, none missing. The resulting per component abnormal rate matches the source paper's own Table 2 exactly for every region already cross checked there, head 8.03%, pelvis 12.3%, vertebra 9.85%, a real check, not just a successful join.
...
## Vertebra anchored search, rerun at full dataset scale

Reran with `src/matching/vertebra_anchored_shoulder_full.py`, all 2925 ids rather than the 20 id sample, same positional outlier check used everywhere else, results in `result/tables/vertebra_anchored_shoulder_full.xlsx`. This is the concrete test that mattered, whether the outlier rate itself drops, not just the aggregate quality means.

It drops substantially. Outlier fraction, before this band versus after, using the centroid relative check as the before number since it is the more principled of the two earlier position checks: shoLANT 16.7% to 4.4%, shoRANT 15.5% to 4.2%, shoLPOST 29.9% to 11.5%, shoRPOST 31.4% to 12.3%. Anterior view is now close to the range every other folder sits in, under about 4%. Posterior view is much improved but still clearly elevated, consistent with the posterior view weakness found earlier in the left-right and far-away mismatch checks. This is a real, substantial improvement, not a full fix, particularly for the posterior view, but it is the first change that actually moves shoulder toward the other 24 folders rather than only explaining why it is hard.
...
## Looking at ref 37's own figures directly

Read the text describing ref 37's figures earlier, never actually looked at them, PyMuPDF renders the pages directly with no need for poppler, unlike the Read tool's own PDF path. Looked at Fig. 10 and Fig. 15.

Fig. 10 shows the shoulder region's actual boundary, not just the text description of it. It has a diagonal cut at the top inner corner, exactly at the neck to shoulder junction, matching the diagonal black wedge already seen repeatedly in this project's own shoulder crops. That wedge is not an artifact of this project's own pipeline, it is what this reference method's own shoulder region looks like, and it plausibly reflects the real anatomical edge of the body there, the arm separating from the torso, rather than an artificial mask painted over real content.

Fig. 15 shows all 23 regions on both views at once, and this is the more useful of the two. Shoulder sits in a junction zone, bordered by head above, humerus below and lateral, vertebra medially, and the unlabeled leftover thorax space below and medial, there is no separate numbered thorax box at all in this figure, thorax is only whatever trunk space is left over once every other named region is removed, matching this project's own chest to thorax mapping from earlier. Shoulder is the one region bordered by four different neighboring structures rather than a self contained bony feature the way pelvis or vertebra are.
