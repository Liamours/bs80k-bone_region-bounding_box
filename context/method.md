...
- Whether masking out the near-zero background pixels before correlating, OpenCV `matchTemplate` takes an optional mask, helps chest/shoulder more than constraining the search area did, not tried yet, motivated by the shoRANT id 1821 case above where 66% of the crop is background
...
Baseline template matching is implemented and has been run on a 20-id sample across all 26 region folders, see "Baseline results" above. It works essentially perfectly for 18 of 26 folders. Vertebra's lower score is explained, a real shape mismatch against a fully textured, non-rectangular crop, with a clear peak. Chest and shoulder's lower score is still open, a constrained-search fix attempt made shoulder worse rather than better, and inspecting one shoulder case directly showed neither the constrained nor unconstrained match reproduces the real crop, whose own content is roughly two thirds blank background, see "Inspecting one shoulder case directly" above. Masking the background before correlating is the next idea, not tried yet. Not yet run on the full dataset. `src/eval/crop_match_metrics.py`, the match scoring module used throughout, is implemented and its own self test has been run against synthetic arrays, see the section above that.
