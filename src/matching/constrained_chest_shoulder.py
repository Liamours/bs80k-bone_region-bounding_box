# /// script
# requires-python = ">=3.11"
# dependencies = ["opencv-python-headless", "numpy", "pillow", "scikit-image", "pandas", "openpyxl"]
# ///
"""Chest/shoulder had a low second peak margin on the full image search, genuine ambiguity,
not just a shape mismatch (see context/method.md). head and pelvis match essentially
perfectly on their own, so use their matched positions as a per-patient vertical band and
restrict the chest/shoulder search to between them, instead of searching the whole image.
"""
import random
import sys
from pathlib import Path

import numpy as np
import pandas as pd
from PIL import Image

sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "eval"))
from crop_match_metrics import compare
from core import locate

RAW = Path(r"C:\Users\lulay\Desktop\wbbs-dataset\bs80k-imaging-raw")
OUT = Path(__file__).resolve().parents[2] / "result" / "tables" / "constrained_chest_shoulder.xlsx"
N = 20
PARTS = ["chestL", "chestR", "shoL", "shoR"]

shared_ids = sorted({int(p.stem) for p in (RAW / "headANT").glob("*.jpg")})
ids = random.Random(0).sample(shared_ids, N)

rows = []
for view in ["ANT", "POST"]:
    whole_dir = RAW / f"wholeBody{view}"
    for i in ids:
        whole = np.asarray(Image.open(whole_dir / f"{i}.jpg"))

        head = np.asarray(Image.open(RAW / f"head{view}" / f"{i}.jpg"))
        head_m = locate(head, whole)
        band_top = head_m["y"] + head_m["h"]

        pelvis = np.asarray(Image.open(RAW / f"pelvis{view}" / f"{i}.jpg"))
        pelvis_m = locate(pelvis, whole)
        band_bottom = pelvis_m["y"]

        for part in PARTS:
            name = f"{part}{view}"
            crop = np.asarray(Image.open(RAW / name / f"{i}.jpg"))
            h = crop.shape[0]

            band = whole[band_top:band_bottom]
            banded = band.shape[0] >= h
            m = locate(crop, band, y_offset=band_top) if banded else locate(crop, whole)

            window = whole[m["y"]:m["y"] + m["h"], m["x"]:m["x"] + m["w"]]
            metrics = compare(crop, window)

            rows.append({
                "component": name, "id": i, "x": m["x"], "y": m["y"], "banded": banded,
                "match_score": m["score"], "peak_margin": m["peak_margin"],
                **metrics,
            })

df = pd.DataFrame(rows)

summary = df.groupby("component").agg(
    n=("id", "count"),
    banded_count=("banded", "sum"),
    match_score_mean=("match_score", "mean"),
    peak_margin_mean=("peak_margin", "mean"),
    near_exact_fraction_mean=("near_exact_fraction", "mean"),
    ssim_mean=("ssim", "mean"),
).reset_index()

OUT.parent.mkdir(parents=True, exist_ok=True)
with pd.ExcelWriter(OUT) as writer:
    df.to_excel(writer, sheet_name="per_sample", index=False)
    summary.to_excel(writer, sheet_name="summary", index=False)

print(f"saved {OUT}, {len(df)} matches")
print(summary.to_string(index=False))
